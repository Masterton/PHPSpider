...
class Curl
{
    ...
    public static function init()
    {
        ...
        if (!is_resource(self::$ch)) {
            ...
            curt_setopt(self::$ch, CURLOPT_NOSIGNAL, true);
        }
        return self::$ch;
    }
     ...
     * 第三种是post stream(json rpc,其实即使webservice)
     * 虽然是post方式,但是只能用流方式 http://input 后者 $HRRP_RAW_POST_DATA 获取
     *
     * @param mixed $url
     * @param array $fields
     * @param mixed $proxy
    ...
    public static function http_request($url, $type = 'get', $fields)
    {
        ...
        if (self::$hosts) {
            ...
            $host = $parse_url['hosts'];
            $key = rand(0, count(self::$hosts) - 1);
            $ip = self::$hosts[$key];
            $url = str_replace($host, $ip, $url);
            self::$headers = array_map(array('Host:' . $host), self::$headers);
        }
    ...
    function classic_curl($urls, $delay)
    {
        ...
        foreach ($urls as $url) {
            ...
            // 吧当前 curl resources 加入到 curl_multi_init 队列
    ...
    function callback($data, $delay, $url)
    {
        ...
        // usleep 模拟现实中比较负责的数据处理逻辑(如提取, 分词, 写入文件或数据库等)
        // usleep(1);
