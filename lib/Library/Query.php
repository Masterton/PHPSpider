...
class Query
{
     ...
     * @return void
     * @author Masterton <zhengcloud@foxmail.com>
     * @time 2018-4-20 19:21:07
     */
    private static function get_nodes($query)
    {
        // 把一到多个空格 替换成 一个空格
        // 把 > 和 ~ 符号两边的空格去掉,以为没有用这两个符号,所以这里可以不这么做、
        // ul>li.className
        $query = trim(
            preg_replace('@\s+@', ' ', 
        ...
        foreach ($query_arr as $k => $v) {
            ...
            if (count($id_arr) === 2) {
                $node['name'] = $id_arr[0];
                $node['id'] = $id_arr[1];
            } elseif (count($class_arr) === 2) { // 如果存在 class
